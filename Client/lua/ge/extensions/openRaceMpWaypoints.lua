...
-- Modified by: irgendeinich for OpenRaceMP
...
local logTag = 'openRaceMP_waypoints'
...
local function onScenarioVehicleTrigger(vid, wpData, dtOff)
  ...
  if scenario.rollingStart and w.cur == 0 then -- hit the starting line
    ...
    if (bo.playerUsable == true or bo.playerUsable == '1') then
      ...
      extensions.hook( 'onRaceWaypointORM', data)
    end
  elseif w.cur ~= -1 then
  ...
  if lapDiff and lapDiff > 0 then
    ...
    if scenario.lapCount > 1 then
      local currentLap = ( math.min(w.lap + 1, scenario.lapCount ) )
      guihooks.trigger('openRaceMp.raceLapChange', {current = currentLap, count = scenario.lapCount } )
    elseif scenario.lapCount == 0 then
      currentLap = data.lap + 1
      guihooks.trigger('openRaceMp.raceLapChange', nil )
    end
    copyInitialWaypointConfig(vid)
  end
end
...
local function activateWaypointBranch(branchName, vehicleId, dtOff)
  ...
  extensions.hook( 'onRaceBranchChosenORM', {
...
local function getLastElementsFromBranch(branchName)
  local ret = {}
  local branchWaypoint = M.state.scenario.lapConfigBranches[branchName]
...
local function initialise(scenario)
  ...
  guihooks.trigger('openRaceMp.raceLapChange', {current = 1, count = scenario.lapCount} )
end
